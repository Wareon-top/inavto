...
/* На старте продвигаем только города, для которых подготовлен отдельный
   полезный сценарий доставки. Остальные страницы остаются доступными людям,
   но не расходуют краулинговый ресурс Google на почти одинаковый контент. */
const PRIORITY_CITY = {
  moskva: {
    intro: 'Доставка автомобиля из Китая в Москву рассчитывается с учётом длинного российского плеча: сравниваем железную дорогу и автовоз, заранее фиксируем итоговую смету и согласуем точку выдачи в Москве или Московской области.',
    route: 'Для московского направления обычно сравниваем железнодорожную отправку с последующим автовозом и прямой автовозный маршрут от пограничного терминала. Выбор зависит от площадки продавца, габаритов автомобиля и графика отправок.',
    features: [
      ['Расчёт до Москвы', 'В смете отдельно показываем цену автомобиля, международную и российскую логистику, таможенные платежи, оформление СБКТС/ЭПТС и комиссию.'],
      ['Выдача в регионе', 'До выкупа согласуем удобный вариант: получение на площадке перевозчика либо доставка автовозом до адреса в Москве или области.'],
      ['Что чаще выбирают', 'Для Москвы чаще запрашивают семейные гибриды, премиальные электромобили и бензиновые кроссоверы — подберём версию под пробег и зарядную инфраструктуру.'],
    ],
  },
  'sankt-peterburg': {
    intro: 'Для Санкт-Петербурга заранее планируем не только путь из Китая, но и длинный финальный участок по России. В договоре фиксируем смету и предельный срок, а на маршруте присылаем статусы погрузки, таможни и движения перевозчика.',
    route: 'Автомобиль идёт через сухопутную границу или Владивосток, затем железной дорогой и автовозом до Санкт-Петербурга. Перед выкупом сравниваем доступные отправки, чтобы не переплачивать за скорость без реальной выгоды.',
    features: [
      ['Контроль длинного маршрута', 'Страхование действует на всём пути, а клиент получает фото и статус после ключевых этапов — от осмотра в Китае до отправки в Санкт-Петербург.'],
      ['Финальная доставка', 'Заранее согласуем получение на площадке перевозчика или доставку до адреса в Санкт-Петербурге и Ленинградской области.'],
      ['Подбор под эксплуатацию', 'Учитываем зимний запас хода, тип привода и возможность домашней зарядки — это помогает выбрать между электромобилем, EREV, PHEV и бензиновой версией.'],
    ],
  },
  novosibirsk: {
    intro: 'Новосибирск находится на основном сибирском направлении поставок из Китая, поэтому здесь особенно важно выбрать удачную отправку и не добавлять лишние перегрузки. Показываем маршрут и полную цену до города ещё до договора.',
    route: 'После пересечения границы автомобиль отправляется по железной дороге либо автовозом через сибирские логистические узлы. Для Новосибирска российское плечо обычно короче, чем для европейской части страны, но срок зависит от очереди на границе и лаборатории.',
    features: [
      ['Маршрут по Сибири', 'Сравниваем железную дорогу и автовоз по ближайшим подтверждённым отправкам, а не называем один способ доставки для всех автомобилей.'],
      ['Получение автомобиля', 'Передаём комплект документов для регистрации и согласуем выдачу в Новосибирске либо дальнейшую перевозку по Новосибирской области.'],
      ['Выбор для зимы', 'При подборе обращаем внимание на полный привод, тепловой насос, ёмкость батареи и реальный зимний пробег конкретной комплектации.'],
    ],
  },
  ekaterinburg: {
    intro: 'Для Екатеринбурга рассчитываем поставку как отдельный маршрут до Урала: учитываем доступные железнодорожные отправки, автовоз и стоимость последней мили. Итоговая сумма до города фиксируется в договоре.',
    route: 'Автомобили для Екатеринбурга идут через сухопутные переходы с дальнейшей отправкой по железной дороге или автовозом. Конкретную схему выбираем после проверки местонахождения машины и расписания ближайших партий.',
    features: [
      ['Смета до Урала', 'В расчёте нет абстрактной «доставки по России»: показываем стоимость маршрута именно до Екатеринбурга и согласованный способ выдачи.'],
      ['Документы к выдаче', 'К автомобилю передаются договор, инвойс, таможенные документы, СБКТС и электронный ПТС — пакет для постановки на учёт.'],
      ['Подбор комплектации', 'Сопоставляем цену в Китае, мощность для расчёта утильсбора, тип привода и оснащение для уральского климата.'],
    ],
  },
  krasnodar: {
    intro: 'Доставка в Краснодар требует точного расчёта длинного пути по России. До подписания договора сравниваем сухопутный и комбинированный маршрут, включаем финальную перевозку в смету и фиксируем предельный срок.',
    route: 'В зависимости от города покупки автомобиль идёт через сухопутную границу либо морем до Владивостока, затем железной дорогой и автовозом на юг России. Выбираем вариант по подтверждённым отправкам и полной стоимости до Краснодара.',
    features: [
      ['Полная цена до Краснодара', 'Смета включает все участки логистики, таможенные платежи, лабораторию, ЭПТС и комиссию — доплаты за финальный участок не появляются при выдаче.'],
      ['Выдача на юге', 'Согласуем получение в Краснодаре или перевозку по Краснодарскому краю; точка и формат передачи фиксируются до отправки.'],
      ['Модели под регион', 'Можно сравнить городской электромобиль, гибрид для дальних поездок и бензиновый кроссовер — считаем полную стоимость каждой версии.'],
    ],
  },
}

export const PRIORITY_CITY_SLUGS = Object.freeze(Object.keys(PRIORITY_CITY))
...
function geoHTML(city, i) {
  ...
  const citySeo = PRIORITY_CITY[slug] || null
  ...
  const intro = citySeo?.intro || GEO_INTROS[i % GEO_INTROS.length](city)
  const route = citySeo?.route || region.route

  const faq = [
    [`Сколько ехать автомобилю из Китая ${inCity}?`,
      `Ориентировочно ${region.term} с момента выкупа. ${route} Максимальный срок фиксируется в договоре.`],
  ...
  const priorityCities = CITIES.filter((c) => PRIORITY_CITY_SLUGS.includes(c[0]))
  const others = priorityCities.filter((c) => c[0] !== slug)
...
<head>
  ...
  <title>${esc(title)}</title>
  <meta name="description" content="${esc(desc)}">
${citySeo ? '' : '  <meta name="robots" content="noindex,follow">\n'}\
...
${citySeo ? `  <section class="section" style="padding-top:0">
    <div class="container">
      <div class="section-head">
        <div class="divider-label">Особенности направления</div>
        <h2 class="h2" style="margin-top:14px">Что важно при доставке ${esc(inCity)}</h2>
      </div>
      <div class="grid grid-3">
        ${citySeo.features.map(([heading, text]) => `<div class="feature reveal"><h3>${esc(heading)}</h3><p>${esc(text)}</p></div>`).join('\n        ')}
      </div>
    </div>
  </section>\n` : ''}\
  ...
  <section class="section" style="padding-top:0">
    <div class="container">
      ...
      <div class="cars-grid" data-geo-cars></div>
  ...
  <script>
    window.INAVTO_PAGE_INIT = function () {
      ...
      if (grid && A.CATALOG_FROM_API) grid.innerHTML = A.CARS.slice(0, 6).map(A.carCard).join('');
    };
  </script>
  <script src="js/main.js"></script>
</body>
...
export function sitemapXML(carSlugs) {
  const urls = [
    ...
    ...PRIORITY_CITY_SLUGS.map((slug) => `${DOMAIN}/gorod/${slug}.html`),
