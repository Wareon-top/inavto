...
test('delivery stories are managed separately and surface only real published videos', () => {
  const db = read('backend/src/db.js')
  const route = read('backend/src/routes/deliveryStories.js')
  const admin = read('backend/public/admin.html')
  const home = read('site/index.html')
  const list = read('site/delivery-stories.html')
  const detail = read('site/delivery-story.html')
  const main = read('site/js/main.js')

  assert.match(db, /CREATE TABLE IF NOT EXISTS delivery_stories/)
  assert.match(route, /router\.get\('\/'/)
  assert.match(route, /router\.get\('\/all', adminOnly/)
  assert.match(route, /router\.post\('\/', adminOnly/)
  ...
  assert.match(detail, /data-delivery-story/)
  assert.match(main, /loadDeliveryStories/)
  assert.match(main, /delivery-story\.html\?slug=/)
})
