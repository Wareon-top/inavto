...
/* Статья о плановых коэффициентах утильсбора должна появиться и на уже
   работающей базе, не меняя карточки и обложки прежних публикаций. */
db.prepare(`
  INSERT OR IGNORE INTO blog_posts (slug, title, excerpt, category, read_time, sort)
  VALUES (?, ?, ?, ?, ?, ?)
`).run(
  'utilsbor-2027-2030',
  'Что будет с утильсбором в 2027–2030 годах: есть ли смысл ждать?',
  'Почему действующий график не обещает снижения и чем рискует покупатель, откладывая заказ автомобиля на год или два.',
  'Деньги',
  '5 мин',
  1,
)
